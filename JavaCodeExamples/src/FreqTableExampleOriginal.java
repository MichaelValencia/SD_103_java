...
public class FreqTableExampleOriginal {
	...
	public static void main(String[] args)
	{
		...
		if( freqs.length == 0)
			...
			String histogram = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
			for(int i = 0; i < freqs.length; i++){
				System.out.printf("char code: %3d ,character: %c ,frequency: %7d - %s\n", i, (char)i, freqs[i], histogram.substring(0,(freqs[i]*100)/max));
			}
			System.out.println("Total characters in file: " + sum(freqs));
		}
	}
